      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <header className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-display text-lg leading-tight">LEAGUE Reviews</div>
              <div className="text-xs text-muted-foreground">Monthly instructor-to-guardian progress updates</div>
            </div>
          </div>
        ...
        <main className="mt-10 grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-12">
          <section className="animate-in">
            ...
            <p className="mt-5 max-w-xl text-base text-muted-foreground sm:text-lg">
              Each month, instructors see their assigned students, write a structured progress update,
              send it to account managers/guardians, and admins get a clean compliance view—without chasing anyone.
            </p>

            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              <a href="/api/login" data-testid="landing-cta-login" className="inline-flex">
                <Button
                  className="w-full rounded-2xl bg-gradient-to-r from-primary to-primary/85 shadow-md shadow-primary/20 hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 sm:w-auto"
                >
                  Get started
                  <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
            ...
            <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="rounded-3xl border bg-card/60 p-4 shadow-sm backdrop-blur">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-sm font-semibold">Instructor workflow</div>
                </div>
          ...
          <section className="animate-in stagger-2">
            <div className="glass rounded-[2rem] p-6 shadow-soft">
              <div className="flex items-center justify-between">
                <div className="font-display text-xl">A month, in one board</div>
              ...
              <div className="mt-6 rounded-3xl border bg-card/60 p-4 shadow-sm">
                <div className="text-sm font-semibold">Template placeholders supported</div>
                <div className="mt-2 flex flex-wrap gap-2 text-xs">
                  {["{{student_name}}", "{{month}}", "{{instructor_name}}", "{{progress_summary}}", "{{next_steps}}"].map((t) => (
                    <span
                      key={t}
        ...
        <footer className="mt-12 border-t pt-6 text-xs text-muted-foreground">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>© {new Date().getFullYear()} LEAGUE of Amazing Programmers</div>
            <div className="flex items-center gap-4">
              <a className="hover:underline" href="/api/login" data-testid="landing-footer-login">
                Login
              </a>
              <Link className="hover:underline" href="/admin" data-testid="landing-footer-admin">
                Admin
              </Link>
            </div>
          </div>
